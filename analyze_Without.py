...
labell=[r"$m_{\rm{base}}$",r"$\Delta A$",r"$\rm{FWHM}$",r"$\Delta_{\rm{TH}}$", r"$T_{\rm{max}}$",r"$\rho_{\star}$",r"$t_{\star}$", r"$u_{\rm{r}}$",r"$f_{\rm{b}}$",r"$m_{\star}$"]
...
fif.close()
f1=open("./paramall1.txt","r")
nm= sum(1 for line in f1) 
par=np.zeros((nm,10)) 
par= np.loadtxt("./paramall1.txt")
##########################################################################
x=np.zeros(( len(df.tstar) , 5))
y=np.zeros(( len(df.tstar) , 5))
for i in range(len(df.tstar)):  
    x[i,0], x[i,1], x[i,2], x[i,3], x[i,4]= df.mbase[i], df.DeltaA[i], df.FWHM[i], df.devi[i], df.Tmax[i]
...
np.savetxt(fif,model.feature_importances_.reshape((-1,5)),fmt="importance &  $%.3f$ &   $%.3f$   &  $%.3f$   &  $%.3f$ &  $%.3f$") 
...
plt.savefig("Example_mbase.jpg",  dpi=200) 
##################################################################
...
ax= plt.gca()              
plot=plt.scatter(abs(par[:,6]),abs(par[:,2]),  c=par[:,7],  cmap=cmap, s=1.5 )
...
fig= plt.figure(figsize=(8,6))
ax= plt.gca()              
plot=plt.scatter(abs(par[:,6]),abs(par[:,4]), c=par[:,7], cmap=cmap, s=1.4)
cb=plt.colorbar(plot)
cb.ax.tick_params(labelsize=16)
cb.set_label(r"$ u_{0}/\rho_{\star}$", fontsize=17)
plt.xticks(fontsize=17, rotation=0)
...
fig= plt.figure(figsize=(8,6))
ax= plt.gca()              
plot=plt.scatter(abs(par[:,7]),abs(abs(par[:,2])/2.0-abs(par[:,4])), c=np.log10(par[:,7]), cmap=cmap)
cb=plt.colorbar(plot)
cb.ax.tick_params(labelsize=16)
...
plt.grid(linestyle='dashed')
fig=plt.gcf()
fig.savefig("./lights/scatter3.jpg",dpi=200)
print("Scatter plots were made *****************************")

###################################################################

for i in range(nm): 
    mbase,  DeltaA,   FWHM,  devi, Tmax=par[i,0], par[i,1], par[i,2], par[i,3], par[i,4]
    rho, tstar,  ur,   fb,  mstar=      par[i,5], par[i,6], par[i,7], par[i,8], par[i,9]
    if(i%10000==0):  
        print("observable features:  ", mbase, DeltaA,  FWHM,  devi,  Tmax )
        print("Lensing parameters:  ",  rho,  tstar, ur,  fb, mstar )
        ...
        if(nd>0):  
            ...
            py.xlim([ 0.0 , 6.0 ])
            fig=plt.gcf()
            fig.savefig("./lights/light_{0:d}.jpg".format(i),dpi=200)
            ###########################################################3
            plt.clf()
            fig= plt.figure(figsize=(8,6)) 
            ...
            plt.yscale('log')
            fig=plt.gcf()
            fig.savefig("./lights/derivative_{0:d}.jpg".format(i),dpi=200)            
